...
2. **HTTP** — `POST /api/v1/mutations/extract` (single) or `/extract-batch` (multiple) with `multipart/form-data`. Schema explorer at <http://localhost:8000/docs>.
...
# 3. Start the server
.venv/bin/uvicorn ocr_mutasi.api:app --host 127.0.0.1 --port 8000 --reload
...
curl -X POST $(for f in mutasi_haswin/Mutasi_*.pdf; do echo -n "-F files=@$f "; done) \
  "http://127.0.0.1:8000/api/v1/mutations/extract-batch" \
...
```bash
.venv/bin/uvicorn ocr_mutasi.api:app --host 0.0.0.0 --port 8000 --workers 4
```
...
##### a) Built-in upload page — `http://localhost:8000/upload` *(recommended for testing)*

The simplest path. Open the URL in a browser, click **Choose PDFs**, multi-select files in the OS dialog (Cmd-click on macOS, Ctrl-click on Windows/Linux, Shift-click to range-select), and click **Extract**. The bare URL `http://localhost:8000/` redirects here too.
...
##### b) Swagger UI (`http://localhost:8000/docs`)
...
curl -X POST -F "file=@contoh_mutasi.pdf" \
  "http://127.0.0.1:8000/api/v1/mutations/extract?classify=true" \
  | jq .
```

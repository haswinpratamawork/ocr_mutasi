...
| 3 | `ECUTI` / `INSENTIF` / `INCENTIVE` / `KOMISI` / `COMMISSION`; **or** any `TUNJANGAN <kind>` other than Hari Raya; **or** `LLG-DEUTSCHE BANK` / any `LLG ` prefix (Lalu Lintas Giro — BI bulk-clearing channel for allowance disbursement). Rule fires before rule 4 so a mixed `KR OTOMATIS LLG-DEUTSCHE BANK` description resolves to Insentif. | **Insentif** |
| 4 | Explicit payroll-disbursement label **and a corporate sender** (`PT <X>` / `<X> PT` / `<X> INDO` / `<X> BSD` / `KASTARA <X>` / etc.). Three flavours: (a) employer-facing keyword (`GAJI` / `PAYROLL` / `SALARY` / `TRSF GAJI` / `PAYROLL-DEPOSIT` / `SALARY-CRDT`); (b) bank bulk-payroll product label (`SAP-DD` / `KR OTOMATIS` *(only when no `LLG`)* / `SMEMFTS`); (c) **professional-fee / honorarium label** (`FEE DOKTER` / `FEE DRG` / `FEE NOTARIS` / `FEE KONSULTAN` / `FEE [profession]` / `HONOR` / `HONORARIUM` / `JASA` / `RETAINER`). **Hard exclusion (overrides any match):** `CASHBACK` / `REFUND` / `REIMBURSE` / `BUNGA` / `TAX REFUND` / `PROMO` always go to Lainnya — these are merchant/bank disbursements, not employer payments. | **Gaji** |
| 5 | *(batch only)* no label match, but **the same corporate sender** appears in ≥ 2 uploaded months. Sender consistency — not amount equality — drives this rule, because real salaries vary monthly (overtime, deductions, prorated months, bundled THR/bonus). Same hard exclusion as rule 4 applies: `CASHBACK` / `REFUND` / `BUNGA` / `PROMO` → Lainnya. | **Gaji** |
| 6 | otherwise | **Lainnya** |

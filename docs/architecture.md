...
| 4 | Explicit payroll-disbursement label — either an employer-facing keyword (`GAJI` / `PAYROLL` / `SALARY` / `TRSF GAJI` / `PAYROLL-DEPOSIT` / `SALARY-CRDT`) or an Indonesian bank bulk-payroll product label (`SAP-DD` / `KR OTOMATIS` / `SMEMFTS` / `LLG-DEUTSCHE BANK` / any `LLG ` prefix). The prompt explicitly biases toward Gaji when one of these labels appears alongside a corporate sender (`PT <X>`, `<X> INDO`). | **Gaji** |
| 5 | *(batch only)* no label match, but **the same corporate sender** appears across multiple uploaded months. Sender consistency — not amount equality — drives this rule, because real salaries vary monthly (overtime, deductions, prorated months, bundled THR/bonus). | **Gaji** |
| 6 | otherwise | **Lainnya** |
...
| **v0.7** | 2026-06-01 | **Fixes a real-world miss on bulk-payroll deposits.** Investigating a 12-month BCA account with 301 credits surfaced two issues: (a) the default `LLM_REQUEST_TIMEOUT_S=30` couldn't complete a 300-credit cross-month call, so every credit fell back to `category: null` and bucketed into Lainnya; (b) the user's actual monthly salary arrived via `SMEMFTS` (BCA SME Mass Funds Transfer Service) and `LLG-DEUTSCHE BANK` (Lalu Lintas Giro) channels, which weren't in rule 4's keyword list — so even when the LLM completed, those ~Rp 180M of monthly salary deposits were misclassified as Lainnya. **Changes:** (1) Default `LLM_REQUEST_TIMEOUT_S` raised from 30 to 120 seconds (batch internally doubles to 240). (2) Rule 4 extended with `SMEMFTS`, `LLG-DEUTSCHE BANK`, and the generic `LLG ` prefix as payroll-disbursement labels, with explicit prompt guidance that when one of these labels appears alongside a corporate sender (`PT <X>` / `<X> INDO`) the row is Gaji. (3) Rule 5 (cross-month recurrence) rewritten to key on **sender consistency** rather than amount equality — real salaries vary monthly due to overtime, deductions, prorated months, and bundled THR/bonus, so requiring "same amount" was too restrictive. (4) `.env.example` updated to 120s. **Validation:** the BCA 12-month batch went from 0 Gaji detections (timeout) → 37 Gaji detections covering all 14 SMEMFTS monthly salary deposits at confidence 1.0, with the LLM citing *"SMEMFTS and PT <X> → Gaji per rule 4"* as the reason. Regression on BCA/Mandiri single-month samples unchanged. |

...
| 4 | Explicit payroll-disbursement label not already caught by rule 3 — either an employer-facing keyword (`GAJI` / `PAYROLL` / `SALARY` / `TRSF GAJI` / `PAYROLL-DEPOSIT` / `SALARY-CRDT`) or an Indonesian bank bulk-payroll product label (`SAP-DD` / `KR OTOMATIS` *(only when no `LLG` is present)* / `SMEMFTS`). The prompt explicitly biases toward Gaji when one of these labels appears alongside a corporate sender (`PT <X>`, `<X> INDO`). | **Gaji** |
